<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Validación básica
        $cantidad_inicial = $quantity; // Inicializar antes de la validación
        ...
        if ($product_name && $price >= 0 && $quantity >= 0 && $quantity > 0) {
            ...
            if (!$stmt) {
                die("Error en prepare: " . $mysqli->error . "<br>Query: INSERT INTO products (product_name, sku, price, quantity, category_id, supplier_id, description, barcode, image, tipo_gestion, cost_price, min_stock, max_stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            }
            $stmt->bind_param("ssdiissssssdd", $product_name, $sku, $price, $cantidad_inicial, $category_id, $supplier_id, $description, $barcode, $image_path, $tipo_gestion, $cost_price, $min_stock, $max_stock);
            if ($stmt->execute()) {
                ...
                $stmt_mov->execute();
                $stmt_mov->close();
                // Actualizar stock usando el procedimiento
                $mysqli->query("CALL sp_update_product_stock($new_product_id)");
            } else {
