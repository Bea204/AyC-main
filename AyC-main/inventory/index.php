...
$query .= " GROUP BY p.product_id, p.product_name, p.sku, p.price, p.quantity, p.category_id, p.supplier_id, p.description, p.barcode, p.image, p.tipo_gestion, p.cost_price, p.min_stock, p.max_stock, p.is_active, p.created_at, p.updated_at, c.name, s.name ORDER BY p.product_name ASC";
...
if (!$stmt) {
    die("Error en prepare: " . $mysqli->error . "<br>Query: " . $query);
}
$stmt->execute();
